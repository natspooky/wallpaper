class s_canvas {
	...
	constructor(canvas, settings) {
		//settings: {fps, resolution:{x,y}, autoResize, useCursor, autoClear, useKey}
		this.#settings = {
			...
			key: this.#fallback(settings.useKey, false),
			devInfo: this.#fallback(settings.devInfo, false),
		};
	...
	setColor(value) {
		if (value.match(/[A-Fa-f0-9]+$/g) && !value.match(/#[A-Fa-f0-9]+$/g)) {
			value = `#${value}`;
		}
	...
	async #frameGen(func, customVar) {
		...
		while (this.#drawing) {
			...
			if (this.#settings.devInfo) {
				this.#devValues(fps, this.#settings.fps, frameDelta);
			}
	...
	#devValues(current, target, delta) {
...
class s_canvas_anim_thread {
	...
	constructor(settings) {
		...
		this.#func = this.cubicBezier; //settings.function;
	}
	...
	cubicBezier(t, p0, p1, p2) {
		return (1 - t) ** 2 * 1 + 2 * (1 - t) * t * 1 + t ** 2 * 1;
	}
}
