class simple_canvas {
	...
	static create(canvasName, settings) {
		// major canvas ssize bug
		const observe = (canvas, settings) => {
			let x = new simple_canvas(canvas, settings);
	...
	setValues() {
		// remeber to remove this if not going to be used
		this.#setValues();
	...
	createAnimation(settings) {
		...
		for (const setting of settings) {
			threads.push(new simple_canvas_anim_thread(setting));
		}
		return threads;
	}
	...
	draw(func, customVar) {
		if (this.#drawing) {
			new Promise((resolve) => {
				resolve(this.#userFunctions.draw);
				this.#drawing = false;
			}).then((val) => {
				if (val)
					this.#userFunctions.draw = this.#frameGen(func, customVar);
			});
		} else {
			this.#userFunctions.draw = this.#frameGen(func, customVar);
		}
	...
	resize(func) {
		this.#userFunctions.resize = func;
	}
	...
	#setEvents() {
		this.#observers.resize = new ResizeObserver(() => {
			this.#setValues();
			if (this.#userFunctions.resize) {
				this.#userFunctions.resize();
			}
		});
...
class simple_canvas_anim_thread {
	#start;
	#current = 0;
	#end;
	#duration;
