class s_canvas {
	...
	setStaticSize(size) {
		this.#canvas.size.set = size;
		this.#calculateSize();
	}
	/*
	animate(initial, final, time) {
		const interval = 
		if (initial >= final) {
			return final;
		}
		...
		while (this.#drawing) {
			...
			if (this.#settings.clear) {
				this.#canvas.context.clearRect(
					0,
					0,
					this.#canvas.body.width,
					this.#canvas.body.height,
				);
			}
